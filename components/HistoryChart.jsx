export default function HistoryChart({ data, height = 140 }) {
  ...
  return (
    <div className="w-full">
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              {d.isToday && (
                <circle cx={cx} cy={padTop + innerH + 12} r="1.5" fill="#136342" />
              )}
              <text
                x={cx}
                y={padTop + innerH + 16}
      ...
      <div className="flex items-center justify-center gap-4 mt-2 font-mono text-[9px] tracking-widest uppercase" style={{ color: "#343434" }}>
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-sm" style={{ background: "#136342" }} />
          Scheduled
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-sm" style={{ background: "#57B98E" }} />
          Snoozed
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-sm" style={{ background: "#bdb5ad" }} />
          Dismissed
        </span>
      </div>
    </div>
